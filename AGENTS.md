...
All four images build from the same Stop Sign checkout and Compose project. The producer's runtime env remains on cube at `/home/drose/manual-apps/stopsign_ai/rtsp_to_redis/.env`; it is referenced by the release Compose file and is not copied into git. Deploy with an explicit scope, for example `~/git/me/domains/mytech/bin/manual-app deploy stopsign --repo-dir ~/git/stopsign_ai --services video_analyzer` for analyzer/database changes, or `--services web_server` for web-only changes. Use all four service names for a coordinated full release; scoped deploys use `--no-deps` and wait for each selected Compose healthcheck plus `/healthz`.
...
- **Labeling**: `scripts/label_passes.py` — resumable, idempotent, 50-way concurrent; ~$0.0004/pass; needs DB_URL, OPENAI_API_KEY, BREMEN_MINIO_* env and `--extra db --extra storage --extra labeling`. Incremental: `scripts/label_increment.sh` runs daily via cube cron (~210 passes/day ≈ $0.08/day); it pulls DB_URL/OPENAI_API_KEY/BREMEN_MINIO_* from the `stopsign-ai` Infisical project at runtime via `infisical run` (same Universal Auth machine identity as the containers), logs to `/home/drose/manual-apps/stopsign/logs/label_increment.log`.
...
## Lookout (operator-armed camera watches)

An operator draws a box on the current clean frame, names what matters, says what they want to be
told, and gets an alert with the frames that decided it. Nothing is trained and no region is coded
at build time: watches are defined at runtime and evaluated by the analyzer.

- **UI**: `/lookout` on the web server, gated by `LOOKOUT_ACCESS_TOKEN` (Infisical `stopsign-ai`,
  env `prod`, injected by `entrypoint.sh`). With no token configured the routes answer **404** rather
  than degrade to open access.
  `infisical secrets get LOOKOUT_ACCESS_TOKEN --projectId 9c373776-768f-454b-a7b3-d1cc40deb475 --env prod --plain`
- **Code**: `stopsign/lookout/` (evaluator; analyzer-only, needs OpenCV), `stopsign/web/routes/lookout.py`
  and `stopsign/web/services/lookout.py` (UI + storage; no OpenCV, must never import the package),
  `static/js/lookout.js`, `scripts/lookout_replay.py`, `tests/test_lookout.py`.
- **Where it runs**: inside `video_analyzer`, on a worker thread fed by a two-slot queue that sheds
  its oldest entry. The frame path only downscales and enqueues — one `_submit_lookout` call in
  `process_frame`, before `visualize()` so the watch sees the frame without burned-in overlays.
  This is not a stylistic choice: the analyzer already runs ~11 fps output at 26.6 ms mean frame
  processing with a ~100 ms p99 and **skips YOLO on stale frames** (14% of frames at last measure),
  so per-frame work does not slow the pipeline gracefully, it starts switching inference off.
- **State**: `watches.json` (definitions; written by the web, read by the analyzer), `events.jsonl`
  and `evidence/<event_id>/` under `/app/data/lookout` (shared `storage-dir` volume); live status in
  Redis `lookout.state`, `lookout.health`, `lookout.frame`, `lookout.references`. Kill switch:
  `redis-cli SET lookout.kill 1` — checked on the frame path, so it can also be released without a restart.
- **Evidence**: `full.jpg` (decisive frame, region marked), `target.jpg`, `strip.jpg` (timestamped
  filmstrip), `summary.json`. Default retention 400 events / 2 days — footage of a public street is a
  liability, so the window is deliberately short.
- **Delivery**: `LOOKOUT_WEBHOOK_URL` POSTs a multipart alert (message + decisive frame) to any
  receiver; point it at `https://ntfy.sh/<topic>` for phone push. Empty means disabled. Logged to
  Redis `lookout.delivery`.

### What it will and will not say

- Presence comes from two references — the appearance the watch was armed on, and the learned
  opposite — plus detections. A detection overlapping the region can raise presence; the *absence*
  of detections cannot lower it (the class list is finite), so emptiness is only ever used to
  confirm an event, never to argue one.
- Resembling neither reference is reported as *unfamiliar content*, not as a decision. Missing
  evidence produces `unknown`/`unavailable`, never the nominal condition.
- Durations are wall-clock seconds with an explicit evidence mask, so dropped or YOLO-skipped frames
  cannot accumulate absence, and a blind watch says so instead of reporting "clear".
- A `cleared` event is worded "clear" only when emptiness was checked for objects; on appearance
  alone it says "no longer the same as when armed". Real footage produced exactly that case (the
  pickup left, a silver SUV took the space), which is why the wording is conditional.
- Decision models (Jev via `LOOKOUT_DECIDER=jev`, a VLM via `vlm`) are **off the alert path**: they
  exist to replay the ambiguous episodes the live rule logs to `arbitration.jsonl`, so "does a
  decision model beat a fixed threshold here?" is a measurement rather than an assumption. The rule
  is the only authority behind an alert.
- Thresholds and cadence all live in `stopsign/lookout/options.py` (env-driven, `LOOKOUT_*`).

### Replay: demo and measurement without waiting for the street

```bash
# Watch a parked vehicle leave its spot, on real archived footage
uv run --extra lookout python scripts/lookout_replay.py /tmp/clip.mp4 \
    --box 854,604,1074,702 --condition cleared --label "stop buffer" \
    --arm-at 3.0 --capture-base 1789943005 --require-alert --json /tmp/report.json
```

`--capture-base` anchors video time to the footage's own clock so evidence stamps match the burned-in
timestamp. Replay of concatenated pass clips is *not* continuous capture and the report says so.
Note that archived pass clips carry burned-in overlays, so replay is degraded relative to live clean
frames; prefer replaying a clean sequence when judging perception quality.

Tests: `make test-lookout` (fast, offline, synthetic) or `make test` (whole suite).

## Implementation Details
- **Capture**: images at the "capture line" crossing (configurable via debug UI); a track the detector only picks up *past* that line - a third of passes, most of them after dark - is photographed at its first usable view instead, so a recorded pass always carries a picture. 40% bbox padding since Aug 2026 (broad archival crop; detection bbox + crop rect recorded in `vehicle_pass_raw.capture` for later tight re-cropping), synchronous local save + async Bremen upload, 1:1 `image_path` mapping
